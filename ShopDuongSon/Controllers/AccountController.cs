...
namespace ShopDuongSon.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model,Customer customer)
        {
            if (!XCaptcha.IsValid)
            {
                ModelState.AddModelError("", "Nhập Mã CapCha không đúng vui lòng nhập lại!");
                return View(model);
            }
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // -- Bắt đầu Xử lý thông tin mở rộng
                    ...
                    {
                        customer.Photo = "user.png";
                    }
                    var db = new DatabaseContext();
                    customer.Id = model.UserName;
                    db.Customers.Add(customer);
                    db.SaveChanges();
                    // -- Kết thúc: Xử lý thông tin mở rộng
